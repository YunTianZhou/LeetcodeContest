...
class Solution:
    def minMergeCost(self, lists: List[List[int]]) -> int:
        ...
        for b in range(n):
            for t in range(1 << b):
                mask = t | (1 << b)
                merged[mask] = sorted(lists[b] + merged[t])
                median[mask] = merged[mask][(len(merged[mask]) - 1) // 2]
        ...
        for mask in range(1, 1 << n):
            ...
            sub = (mask - 1) & mask
            while sub > (mask ^ sub):
                cost[mask] = min(cost[mask], abs(median[sub] - median[mask ^ sub]) + cost[sub] + cost[mask ^ sub])
                sub = (sub - 1) & mask
            cost[mask] += len(merged[mask])
...
if __name__ == "__main__":
    ...
    # Example 2
    print(sol.minMergeCost([[1, 1, 5], [1, 4, 7, 8]]))  # 10

    # Example 3
    print(sol.minMergeCost([[1], [3]]))  # 4

    # Example 4
    print(sol.minMergeCost([[1], [1]]))  # 2
