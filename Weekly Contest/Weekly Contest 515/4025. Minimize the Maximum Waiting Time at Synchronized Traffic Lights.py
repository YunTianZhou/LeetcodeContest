...
class Solution:
    def minPenalty(self, period: int, lights: list[int], arrivalTime: list[int]) -> int:
        mx = max(lights)
        return max((period - x % period for x in arrivalTime if x % period >= mx), default=0)
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.minPenalty(8, [2, 3], [2, 5, 8, 11]))  # 5

    # Example 2
    print(sol.minPenalty(10, [3, 6, 8], [4, 9, 15]))  # 1

    # Example 3
    print(sol.minPenalty(5, [2], [2, 3, 4, 5, 6]))  # 3
