...
class Solution:
    def nearestDrone(self, drones: list[list[int]], target: list[int]) -> int:
        ...
        for i, (x, y, r) in enumerate(drones):
            d = abs(target[0] - x) + abs(target[1] - y)
            if d <= r and d < mn:
                mn = d
                j = i
        return j
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.nearestDrone([[0, 0, 8], [2, 2, 9]], [3, 4]))  # 1

    # Example 2
    print(sol.nearestDrone([[2, 1, 5], [4, 4, 5], [6, 6, 8]], [5, 5]))  # 1

    # Example 3
    print(sol.nearestDrone([[4, 4, 5]], [8, 6]))  # -1
