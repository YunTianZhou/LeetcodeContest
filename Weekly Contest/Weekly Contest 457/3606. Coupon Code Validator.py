...
class Solution:
    def validateCoupons(self, code: List[str], businessLine: List[str], isActive: List[bool]) -> List[str]:
        ...
        valid_indices = [i for i in range(n) if  isActive[i] and 
                         re.match(r"^[a-zA-Z0-9_]+$", code[i]) and businessLine[i] in categories]
        valid_indices.sort(key=lambda i: (categories.index(businessLine[i]), code[i]))
