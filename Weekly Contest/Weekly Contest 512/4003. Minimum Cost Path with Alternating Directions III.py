"""
4003. Minimum Cost Path with Alternating Directions III - Hard
...
class Solution:
    def minCost(self, m: int, n: int, penalty: list[list[int]]) -> int:
        directions = [(1, 0, 0), (0, 1, 0), (-1, 0, 1), (0, -1, 1)]
        
        hp = [(1, 0, 0, 0)]
        dst = [[[inf, inf] for _ in range(n)] for _ in range(m)]
        ...
        while hp:
            ...
            push(i, j, p ^ 1, d + penalty[i][j])
            
            for di, dj, dp in directions:
                ni, nj = i + di, j + dj
                if 0 <= ni < m and 0 <= nj < n:
                    nd = d + (ni + 1) * (nj + 1) + (0 if p == dp else penalty[i][j])
                    push(ni, nj, p ^ 1, nd)
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.minCost(2, 2, [[5, 3], [1, 4]]))  # 8

    # Example 2
    print(sol.minCost(2, 2, [[0, 7], [3, 2]]))  # 7

    # Example 3
    print(sol.minCost(2, 3, [[8, 0, 9], [7, 4, 1]]))  # 12
