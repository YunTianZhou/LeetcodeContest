"""
3947. Maximum Number of Items From Sale II - Medium
...
class Solution:
    def maximumSaleItems(self, items: list[list[int]], budget: int) -> int:
        ...
        for factor, price in items:
            cnt.update(factors[factor])
            mn = min(mn, price)

        groups = [(price, cnt[factor] - 1) for factor, price in items if price < 2 * mn]
        groups.sort()
        
        ans = 0
        for price, amount in groups:
            buy = min(amount, budget // price)
            budget -= buy * price
            ans += buy * 2
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.maximumSaleItems([[1, 6],[2, 4],[3, 5]], 19))  # 5

    # Example 2
    print(sol.maximumSaleItems([[2, 8],[1, 10],[6, 6],[4, 12],[5, 20],[5, 17]], 35))  # 7
