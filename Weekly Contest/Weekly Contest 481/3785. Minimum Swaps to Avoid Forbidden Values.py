...
class Solution:
    def minSwaps(self, nums: List[int], forbidden: List[int]) -> int:
        ...
        cnt_n = Counter(nums)
        cnt_f = Counter(forbidden)

        for x in cnt_n:
            if cnt_n[x] + cnt_f[x] > n:
                return -1
        
        cnt = Counter(x for x, y in zip(nums, forbidden) if x == y)
        if not cnt:
            return 0
        
        return max(max(cnt.values()), (sum(cnt.values()) + 1) // 2)
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.minSwaps([1, 2, 3], [3, 2, 1]))  # 1

     # Example 2
    print(sol.minSwaps([4, 6, 6, 5], [4, 6, 5, 5]))  # 2

    # Example 3
    print(sol.minSwaps([7, 7], [8, 7]))  # -1

    # Example 4
    print(sol.minSwaps([1, 2], [2, 1]))  # 0
