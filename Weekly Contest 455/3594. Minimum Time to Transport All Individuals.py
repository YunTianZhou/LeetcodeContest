...
class Solution:
    def minTime(self, n: int, k: int, m: int, time: List[int], mul: List[float]) -> float:
        full_mask = (1 << n) - 1
        hp = [(0.0, full_mask, 0, False)]  # (total_time, mask, stage, is_return)
        visited = set()

        while hp:
            total_time, mask, stage, is_return = heappop(hp)
            if mask == 0:
                return total_time
            if (mask, stage, is_return) in visited:
                continue
            visited.add((mask, stage, is_return))
            ...
            if is_return:
                for i in range(n):
                    if ~mask & (1 << i):
                        ...
                        if (new_mask, new_stage, False) not in visited:
                            heappush(hp, (total_time + return_time, new_mask, new_stage, False))
            else:
                camp = [i for i in range(n) if mask & (1 << i)]
                for r in range(1, min(k, len(camp)) + 1):
                    for group in combinations(camp, r):  
                        crossing_time = max(time[i] for i in group) * mul[stage]
                        new_mask = mask
                        for i in group:
                            new_mask ^= (1 << i)
                        new_stage = (stage + int(crossing_time)) % m
                        if (new_mask, new_stage, True) not in visited:
                            heappush(hp, (total_time + crossing_time, new_mask, new_stage, True))
...
if __name__ == "__main__":
    ...
    # Example 3
    print(sol.minTime(2, 1, 2, [10, 10], [2.0, 2.0]))  # -1.00000
