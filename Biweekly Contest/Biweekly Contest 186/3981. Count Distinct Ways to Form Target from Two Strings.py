"""
3981. Count Distinct Ways to Form Target from Two Strings - Hard
...
class Solution:
    def interleaveCharacters(self, word1: str, word2: str, target: str) -> int:
        ...
        def dfs(i, j, k):
            ...
            ans = dfs(i, j - 1, k) + dfs(i - 1, j, k) - dfs(i - 1, j - 1, k)
            
            if i > 0 and word1[i - 1] == target[k - 1]:
                ans += dfs(i - 1, j, k - 1) - dfs(i - 1, j - 1, k - 1)
            
            if j > 0 and word2[j - 1] == target[k - 1]:
                ans += dfs(i, j - 1, k - 1) - dfs(i - 1, j - 1, k - 1)
            
            return ans % mod
            
        return (dfs(m1, m2, n) - dfs(m1, 0, n) - dfs(0, m2, n)) % mod
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.interleaveCharacters("abc", "bac", "abc"))  # 5

    # Example 2
    print(sol.interleaveCharacters("cd", "cd", "ccd"))  # 4

    # Example 3
    print(sol.interleaveCharacters("xy", "xy", "xyxy"))  # 2

    # Example 4
    print(sol.interleaveCharacters("ab", "cde", "ace"))  # 1
