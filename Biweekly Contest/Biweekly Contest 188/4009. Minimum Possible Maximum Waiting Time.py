"""
4009. Minimum Possible Maximum Waiting Time - Hard
...
Car  Becomes allowed at  Starts refueling at  Dispenser used  Remaining fuel before start  Waiting time
...
class Solution:
    def maxServedCars(self, demand: list[int], fuel: list[int]) -> int:
        dp = [False] * (fuel[0] + 1)
        dp[0] = True

        sm = mx = 0
        for i, x in enumerate(demand):
            for j in range(fuel[0], x - 1, -1):
                if dp[j - x]:
                    dp[j] = True
                    mx = max(mx, j)
    ...
    def minMaxWaitingTime(self, demand: list[int], fuel: list[int]) -> int:
        pref = list(accumulate(demand, initial=0))
        total = fuel[0] + fuel[1]

        k = self.maxServedCars(demand, fuel)
        if k == 0:
            return -1

        mx_demand = max(demand[:k])

        @cache
        def dfs(i: int, f0: int, t0: int) -> bool:
            if i == k:
                return 0

            f1 = total - f0 - pref[i]
            t1 = demand[i - 1] if i > 0 else 0
            x = demand[i]
            ...
            if x <= f0:
                ans = max(t0, dfs(i + 1, f1, max(0, t1 - t0)))

            if x <= f1 and ans > t1:
                cur = max(t1, dfs(i + 1, f0, max(0, t0 - t1)))
                ans = min(ans, cur)

            return ans
        
        return dfs(0, fuel[0], 0)
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.minMaxWaitingTime([6, 8, 4, 6, 5], [16, 13]))  # 6

    # Example 2
    print(sol.minMaxWaitingTime([10, 15], [12, 17]))  # 0

    # Example 3
    print(sol.minMaxWaitingTime([10, 5], [8, 8]))  # -1
