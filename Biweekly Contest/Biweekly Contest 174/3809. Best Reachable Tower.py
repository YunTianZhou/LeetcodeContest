...
class Solution:
    def bestTower(self, towers: List[List[int]], center: List[int], radius: int) -> List[int]:
        cx, cy = center
        _, x, y = min(((-q, x, y) for x, y, q in towers if abs(cx - x) + abs(cy - y) <= radius), default=(1, -1, -1))
        return [x, y]
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.bestTower([[1, 2, 5], [2, 1, 7], [3, 1, 9]], [1, 1], 2))  # [3, 1]
    
    # Example 2
    print(sol.bestTower([[1, 3, 4], [2, 2, 4], [4, 4, 7]], [0, 0], 5))  # [1, 3]

    # Example 3
    print(sol.bestTower([[5, 6, 8], [0, 3, 5]], [1, 2], 1))  # [-1, -1]
