...
class Solution:
    def minMoves(self, sx: int, sy: int, tx: int, ty: int) -> int:
        ans = 0
        while (tx > sx or ty > sy) and (tx >= sx and ty >= sy):
            if tx < ty:
                tx, ty = ty, tx
                sx, sy = sy, sx
        ...
        return -1 if (sx, sy) != (tx, ty) else ans
...
if __name__ == "__main__":
    ...
    # Example 1
    print(sol.minMoves(1, 2, 5, 4))  # 2

    # Example 2
    print(sol.minMoves(0, 1, 2, 3))  # 3

    # Example 3
    print(sol.minMoves(1, 1, 2, 2))  # -1
